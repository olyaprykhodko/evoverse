...
function buildStrip() {
  ...
  for (const [sym, n] of Object.entries(COUNTS))
    for (let i = 0; i < n; i++) bag.push(sym);
...
for (let n = 0; n < spins; n++) {
  ...
  const grid = REELS.map((reel) => {
    ...
    return [
      reel[stop],
      reel[(stop + 1) % STRIP_LEN],
      reel[(stop + 2) % STRIP_LEN],
    ];
  });
  ...
  for (const line of PAYLINES) {
    ...
    if (symbol && count >= 3) {
      ...
      if (m > 0) {
        ...
        tierCount[`${symbol}-${count}`] =
          (tierCount[`${symbol}-${count}`] || 0) + 1;
      }
    }
  }
  totalBet += BET;
...
console.log(
  `hit freq:     ${((winningSpins / spins) * 100).toFixed(2)}%  (>=1 winning line)`,
);
