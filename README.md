...
При реєстрації автоматично створюється `Wallet` і надсилається лист верифікації email (`emailVerified: false`, див. розділ Auth). `PATCH /users/:id` потребує підтвердженого email (`EmailVerifiedGuard`).
